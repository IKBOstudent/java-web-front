...
const Card = ({ item, listId }) => {
    ...
    const handleDelete = (event) => {
        dispatch(deleteCardById({ listId, cardId: item.cardId }));
        setOpen(false);
    };
    ...
    return (
        <>
            <div
                className="mt-4 p-3 bg-slate-300 min-h-[70px] cursor-pointer"
                onClick={() => setOpen(true)}>
                <span className="text-md">{item.cardName}</span>
            </div>
            {isOpen && (
                <div className="fixed top-0 left-0 w-full h-full bg-black/60 px-4">
                    <div className="flex flex-col gap-2 max-w-[30rem] p-4 bg-slate-50 mx-auto mt-20 shadow-2xl">
                        <div className="flex justify-between w-full">
                            <h1 className="p-2 text-lg font-semibold">{item.cardName} </h1>
                            <button
                                type="button"
